...
load("rdata_nlmr_bmi_20250407.RData")

pheno <- pheno %>%
  mutate(sex = as.numeric(sex) - 1)

pheno <- pheno[, c("bmigrs", "bmi", "towndepriv", "age", "sex", "fi", "met")]
...
for (i in 1:rep) {
  ...
  model5   <- lm(dat$x ~ g * mod5)
  mm5      <- model.matrix(~ g * mod5)
  coefs5   <- coef(model5)
  int5     <- grep("^g:mod5", colnames(mm5))
  int_eff5 <- as.vector(mm5[, int5, drop = FALSE] %*% coefs5[int5])
  dat$x5   <- dat$x - int_eff5
  
  model_all1 <- lm(dat$x ~ g * mod1 + g * mod2 + g * mod3 + g * mod4 + g * mod5)
  ...
  dat$x6 <- dat$x - int_eff_all1
  
  model_all2 <- lm(dat$x ~ g * mod1 * mod2 * mod3 * mod4 * mod5)
  mm_all2    <- model.matrix(~ g * mod1 * mod2 * mod3 * mod4 * mod5)
  coefs_all2 <- coef(model_all2)
  int_cols_all2 <- grep("^g:", colnames(mm_all2))
  int_eff_all2  <- as.vector(mm_all2[, int_cols_all2, drop = FALSE] %*%
                               coefs_all2[int_cols_all2])
  dat$x7 <- dat$x - int_eff_all2
  ...
  temp <- bind_rows(m1, m2, m3, m4, m5, m6, m7, m8, m9, m10) %>%
    mutate(
      lace    = by / bx,
      lace_se = byse / bx
    )
...
save(
  bmi_simNeg,
  file = "rdata_nlmr_bmi_negControl_results_20251206.RData"
)
